...
window.onload = async () => {
    movieList = document.getElementById("main");
...
function addMovie(movie) {
    let card = addNewElement("div", "movie-card", movieList);
    let image = addNewElement("img", "card-img-top", card)
    image.src = imageBaseUrl + movie.backdrop_path;
    let cardTitle = addNewElement("p", "movie-title", card, movie.original_title);
}
